...
CEnemyBillboardFire::CEnemyBillboardFire(float x_,float y_,float z_,float w_,float h_,float d_, int textureID_, bool bUseSpriteSheet_, int cols_, int rows_, int fps_, bool additive_)
...
CEnemyBillboardFire::~CEnemyBillboardFire(void)
{
...
void
CEnemyBillboardFire::PostRender()
{
	glPushMatrix();
	glFrontFace(GL_CCW);
...
void
CEnemyBillboardFire::Update(float dt_)
{
	...
	//if (CTimer::Instance()->GetTime() > 10000)
	//{			
	//	float m_fBulletSpeed=10.0f;

	//	C3dObject * enemyBullet=new CEnemyBullet(m_fX,m_fY,m_fZ,1.0,1.0,1.0,0);
	//	gGame.AddObject(enemyBullet);
	//			
	//	enemyBullet->m_fVelocityx=dx*xDistance * m_fBulletSpeed;
	//	//enemyBullet->m_fVelocityy=dy*yDistance * m_fBulletSpeed;
	//	enemyBullet->m_fVelocityz=dz*zDistance * m_fBulletSpeed;
	//}
	///////////End enemy firing at player///////////////

	CalcBoundingSphere();
}

void
CEnemyBillboardFire::SetAnimFrame(int frame_)
{
...
void
CEnemyBillboardFire::CalcBoundingSphere()
{
